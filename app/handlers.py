...
from db.db import all_tasks, add_task
...
async def all_tasks_handler(message: Message):
    tasks = await all_tasks(message.from_user.id)
    if tasks:
        text = "Ваши задачи:\n"
        for task in tasks:
            text += f"ID: {task.tg_id}\nTitle: {task.title}\nBody: {task.body}\n\n"
...
async def add_task_handler(message: Message, state: FSMContext):
    ...
    await add_task(tg_id=message.from_user.id, title=data.get('title'), body=data.get('body'))
